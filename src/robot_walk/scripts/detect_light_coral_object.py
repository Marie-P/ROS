...
class image_converter:
  ...
  def callback(self,data):  #--- Callback function
    ...
    low_blue = np.array([0,50,50])
    high_blue = np.array([10,255,255])
    blue_mask = cv2.inRange(hsv_frame, low_blue, high_blue)
    pink_mask = cv2.inRange(hsv_frame, (0,50,50), (10,255,255))
    blue = cv2.bitwise_and(cv_image, cv_image, mask=blue_mask)
    # blue = cv2.bitwise_and(cv_image, cv_image, mask=pink_mask)
    # pink = cv2.bitwise_and(cv_image, cv_image, mask=pink_mask)
    pink_resize = cv2.resize(blue, (360, 240)) 
    image_resize = cv2.resize(cv_image, (360, 240)) 
    elementsR=cv2.findContours(pink_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    if len(elementsR) > 0:
    ...
    cv2.namedWindow("Masque rose")
    cv2.moveWindow("Masque rose", 0, 0)
    cv2.imshow("Masque rose", pink_resize)
    cv2.namedWindow("Image camera")
    cv2.moveWindow("Image camera", 0, 400)
    cv2.imshow("Image camera", image_resize)
    cv2.waitKey(1)
  ...
  def callback(self,msg):
    if self.ic.tourne == 1:
      if msg.ranges[0] > 0.16 and msg.ranges[1] > 0.18 and msg.ranges[2] > 0.20 and msg.ranges[3] > 0.22 and msg.ranges[23] > 0.22 and msg.ranges[22] > 0.20 and msg.ranges[21] > 0.18 and msg.ranges[20] > 0.16:
        ...
        self.tourne_gauche()
    elif self.ic.tourne == -1:
      if msg.ranges[0] > 0.16 and msg.ranges[1] > 0.18 and msg.ranges[2] > 0.20 and msg.ranges[3] > 0.22 and msg.ranges[23] > 0.22 and msg.ranges[22] > 0.20 and msg.ranges[21] > 0.18 and msg.ranges[20] > 0.16:
        self.avancer_droite()
      else :
        self.tourne_droite()
    elif self.ic.tourne == 0:
      if msg.ranges[0] > 0.16 and msg.ranges[1] > 0.18 and msg.ranges[2] > 0.20 and msg.ranges[3] > 0.22 and msg.ranges[23] > 0.22 and msg.ranges[22] > 0.20 and msg.ranges[21] > 0.18 and msg.ranges[20] > 0.16:
        self.avancer()
        # print("Avancer !!")
      else:
